	...
	@Override public void action(String[] args, MessageReceivedEvent event)
	{
		...
		if (message.isFromType(ChannelType.TEXT))
		{
			...
			else
			{
				...
				// Now, we the the logged in account doesn't have permission to kick members..
				// well.. we can't kick!
				...
				{
					Member member = guild.getMember(user); // We get the member object for each mentioned user to kick
					// them!

					// We need to make sure that we can interact with them. Interacting with a
					// Member means you are higher
					// in the Role hierarchy than they are. Remember, NO ONE is above the Guild's
					// Owner. (Guild#getOwner())
					if (!selfMember.canInteract(member))
					{
						channel.sendMessage("Cannot kick member: " + member.getEffectiveName() + ", they are higher " + "in the hierachy than I am!").queue();
					...
					guild.getController().kick(member).queue(success -> channel.sendMessage("Kicked " + member.getEffectiveName() + "! Cya!").queue(), error -> {
						...
						if (error instanceof PermissionException)
						{
							...
							// exactly what permission is missing, this is how.
							// Note: some PermissionExceptions have no permission provided, only an error
							// message!
						...
						{
							channel.sendMessage(
									"Unknown error while kicking [" + member.getEffectiveName() + "]: " + "<" + error.getClass().getSimpleName() + ">: " + error.getMessage())
									.queue();
						}
					});
				}
